	...
	<div class="sidebar-shortcuts" id="sidebar-shortcuts">
		...
		if($rright['side_open_ticket'] && (($_GET['page']!='asset_list') || ($rright['side_asset_create']==0)) && ($_GET['page']!='asset')  && ($_GET['page']!='procedure') && ($_GET['page']!='project'))
		{
			...
			<div class="sidebar-shortcuts-large" id="sidebar-shortcuts-large">
					<a href="'.$target_url.'">
						<button style="width:170px" accesskey="n" title="'.T_("Ajoute un nouveau ticket").' (SHIFT+ALT+n)" onclick=\'window.location.href="'.$target_url.'"\' class="btn btn-sm btn-success">
							&nbsp;
		...
		if($rright['side_asset_create'] && ($rparameters['asset']==1) && (($_GET['page']=='asset_list') || ($_GET['page']=='asset') ) && ($_GET['state']!='1'))
		{
			...
			<div class="sidebar-shortcuts-large" id="sidebar-shortcuts-large">
					<a href="./index.php?page=asset&amp;action=new">
						<button style="width:170px" accesskey="n" title="'.T_('Ajoute un nouvel équipement').' (SHIFT+ALT+n)" onclick=\'window.location.href="./index.php?page=asset&amp;action=new"\' class="btn btn-sm btn-success">
							&nbsp;
		...
		if($rright['side_asset_create'] && ($rparameters['asset']==1) && (($_GET['page']=='asset_list') || ($_GET['page']=='asset') ) && ($_GET['state']=='1'))
		{
			...
			<div class="sidebar-shortcuts-large" id="sidebar-shortcuts-large">
					<a href="./index.php?page=asset_stock">
						<button style="width:170px" title="'.T_('Permet l\'ajout de plusieurs équipements à la fois').'" onclick=\'window.location.href="./index.php?page=asset_stock"\' class="btn btn-sm btn-warning">
							&nbsp;
		...
		if($rright['procedure_add'] && ($_GET['page']=='procedure'))
		{
			...
			<div class="sidebar-shortcuts-large" id="sidebar-shortcuts-large">
					<a href="index.php?page=procedure&amp;action=add">
						<button style="width:170px" accesskey="n" title="'.T_('Ajoute une nouvelle procédure').' (SHIFT+ALT+n)" class="btn btn-sm btn-success">
							&nbsp;
		...
		if($rright['project'] && ($_GET['page']=='project'))
		{
			echo'
			<div class="sidebar-shortcuts-large" id="sidebar-shortcuts-large">
					<a href="index.php?page=project&amp;action=add">
						<button style="width:170px" accesskey="n" title="'.T_('Ajoute un nouveau projet').' (SHIFT+ALT+n)" class="btn btn-sm btn-success">
							&nbsp;
							<i class="icon-plus bigger-120"></i> '.T_('Nouveau projet').'
						</button>
					</a>
			</div>
			<div class="sidebar-shortcuts-mini" id="sidebar-shortcuts-mini">
				<a href="index.php?page=project&amp;action=add"><span class="btn btn-success"></span></a>
			</div>
			';
		}
		?>
	...
	<ul class="nav nav-list">
			...
			echo "<li "; if(($_GET['page']=='dashboard' || $_GET['page']=='ticket') && $_GET['userid']!='%' && $_GET['userid']!='0') {echo 'class="active"';} echo ">
				...
				<ul class=\"submenu\" >";
					...
					if ($_GET['userid']!='%' && $_GET['state']=='%') {echo '<li class="active">';} else {echo "<li>";} echo "
						    <a href=\"./index.php?page=dashboard&amp;userid=$_SESSION[user_id]&amp;state=%25&amp;ticket=%25&amp;category=%25&amp;subcat=%25&amp;title=%25&amp;date_create=%25&amp;priority=%25&amp;criticality=%25&amp;company=%25\">
			...
			if ($_GET['page']=='dashboard' && ($_GET['userid']=='%' || $_GET['userid']=='0') && $_GET['viewid']=='' && $_GET['companyview']!='') echo '<li  class="active">'; else echo '<li>'; echo '
				...
				<ul class="submenu" >';
					if ($_GET['page']=='dashboard' && $_GET['userid']=='%' && $_GET['state']=='%') echo '<li class="active">'; else echo '<li>'; echo '
						<a href="./index.php?page=dashboard&amp;userid=%25&amp;state=%25&amp;ticket=%25&amp;technician=%25&amp;user=%25&amp;category=%25&amp;subcat=%25&amp;title=%25&amp;date_create=%25&amp;priority=%25&amp;criticality=%25&amp;companyview=1">
							<i class="icon-double-angle-right"></i>
					...
					{
						$qry=$db->prepare("SELECT COUNT(tincidents.id) FROM `tincidents`,`tusers` WHERE tincidents.user=tusers.id AND tincidents.disable='0' AND (tincidents.state=1 OR tincidents.state=2 OR tincidents.state=6) AND tusers.company=:company");
						$qry->execute(array('company' => $ruser['company']));
		...
		) //not display all tickets for supervisor without service or agency, without user_limit_service tech must view all tickets
		{
			...
			if (($_GET['userid']=='%' || $_GET['userid']=='0') && $_GET['viewid']=='' && $_GET['companyview']=='') echo '<li  class="active">'; else echo '<li>'; echo '
				...
				<ul class="submenu" >';
					if ($_GET['userid']=='%' && $_GET['state']=='%') echo '<li class="active">'; else echo '<li>'; echo '
						<a href="./index.php?page=dashboard&amp;userid=%25&amp;state=%25&amp;ticket=%25&amp;technician=%25&amp;user=%25&amp;category=%25&amp;subcat=%25&amp;title=%25&amp;date_create=%25&amp;priority=%25&amp;criticality=%25&amp;company=%25">
							<i class="icon-double-angle-right"></i>
							'.T_('Tous les états').' ('.$cntall[0].')
						</a>
					</li>';
					...
					if ($cnt5[0]>0 && $rright['side_your_not_attribute']!=0)
					{
						if ($_GET['userid']=='0' && $_GET['state']=='%') echo '<li class="active">'; else echo '<li>'; echo '
							<a href="./index.php?page=dashboard&amp;userid=0&amp;t_group=0&amp;state=%25">
								<i class="icon-double-angle-right"></i>
				...
				if ($rright['side_asset_all_state']!=0)
				{
					...
					<ul class="submenu">
						<li';
							...
							echo '>
								<a title="'.T_('Tous les équipements').'" href="./index.php?page=asset_list&amp;state=%25">
									<i class="icon-double-angle-right"></i>
									'.T_('Tous').' ('.$cnt[0].')
								</a>
						</li>
						...
						while($row=$qry->fetch()) 
						{
							...
							if($rright['asset_list_company_only']!=0)
							{
								$qry2=$db->prepare("SELECT COUNT(tassets.id) FROM `tassets`,`tusers` WHERE tassets.user=tusers.id AND tassets.state LIKE :state AND tassets.disable='0' AND tusers.company=:company");
								$qry2->execute(array('state' => $row['id'],'company' => $ruser['company']));
								$cnt=$qry2->fetch();
								$qry2->closeCursor();
							} else {
								$qry2=$db->prepare("SELECT COUNT(id) FROM `tassets` WHERE state LIKE :state AND disable='0'");
		...
		if($rright['procedure'] && $rparameters['procedure']==1)
		{
			 if($_GET['page']=='procedure') echo '<li class="active">'; else echo '<li>'; echo '
				<a href="./index.php?page=procedure" class="dropdown-toggle">
					<i class="icon-book"></i>
		...
		{
			if($_GET['page']=='calendar') echo '<li class="active">'; else echo '<li>'; echo '
				<a href="./index.php?page=calendar" class="dropdown-toggle">
		...
		if($rright['availability'] && $rparameters['availability']==1)
		{
			if($_GET['page']=='availability') echo '<li class="active">'; else echo '<li>'; echo '
				<a href="./index.php?page=plugins/availability/index" class="dropdown-toggle">
					<i class="icon-time"></i>
		...
		{
			if($_GET['page']=='project') echo '<li class="active">'; else echo '<li>'; echo '
				<a href="./index.php?page=project" class="dropdown-toggle">
					<i class="icon-tasks"></i>
					<span class="menu-text">'.T_('Projets').'</span>
				</a>
			</li>';
		}
		if($rright['stat'])
		{
			if($_GET['page']=='stat') echo '<li class="active">'; else echo '<li>'; echo '
				<a href="./index.php?page=stat&tab=ticket" class="dropdown-toggle">
