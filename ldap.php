...
if(($_GET['action']=='simul') || ($_GET['action']=='run') || $_POST['test_ldap'] || ($_GET['ldap']=='1') || ($ldap_auth==1))
{
	...
	if ($ldapbind) {$ldap_connection="<i title=\"Connecteur opérationnel\" class=\"icon-ok-sign icon-large green\"></i> Connecteur opérationnelle.";} else {$ldap_connection="<i title=\"Le connecteur ne fonctionne pas vérifier vos paramètres\" class=\"icon-remove-sign icon-large red\"></i> Le connecteur ne fonctionne pas vérifier vos paramètres";}
	...
	if ($ldapbind) 
	{
		if(($_GET['action']=='simul') || ($_GET['action']=='run')) 
		{
				...
				for ($i=0; $i < $cnt_ldap; $i++) 
				{
					...
					if($rparameters['debug']==1) echo "SamAccountName=$samaccountname UAC=$UAC<br>";
