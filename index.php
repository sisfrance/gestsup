...
<html lang="fr">
	<head>
		...
		<?php 
		if($_SESSION['user_id'] && $rparameters['auto_refresh']!=0 && $_GET['page']=='dashboard' && $_POST['keywords']=='')
						...
						<div class="navbar-header pull-right" role="navigation">
							<ul class="nav ace-nav">';
								if ($rright['userbar']!=0)
								{
									...
									echo'
									<li class="blue">
										<a title="'.T_('Tickets ouverts, fermés ou sur lesquels un élément de résolution a été ajouté').'" href="./index.php?page=dashboard&amp;userid=%25&amp;state=%25&amp;ticket=%25&amp;technician=%25&amp;user=%25&amp;category=%25&amp;subcat=%25&amp;title=%25&amp;date_create=%25&amp;priority=%25&amp;criticality=%25&amp;company=%25&amp;view=activity&amp;date_start='.date("d/m/Y").'&date_end='.date("d/m/Y").'">
									...
									<li class="grey">
										<a title="'.T_('Vos tickets en attente de prise en charge, en cours et en attente de retour').'" href="'.$link_meta_state.'">
											...
											echo '
											<span title="'.T_('Vos tickets en attente de prise en charge, en cours et en attente de retour').'" class="badge badge-grey">'; if($mobile==0) {echo $nbatt[0];} echo'</span>
										</a>
									</li>
									';
									...
									if ($rright['ticket_time_disp']!=0 && $rright['ticket_time_hope_disp']!=0)
									{
										...
										<li class="purple">
											<a title="'.T_('Nombre d\'heures de travail estimé dans vos tickets ouverts').'" href="./index.php?page=dashboard&amp;userid='.$_SESSION['user_id'].'&amp;state=1">
												...
												echo '
												<span class="badge badge-important">'; if($mobile==0) {echo $nbtps.'h';} echo '</span>
											</a>
										</li>
										';
									}
								...
								<li class="green">
									<a title="'.T_('Vos tickets résolus').'" href="./index.php?page=dashboard&amp;userid='.$_SESSION['user_id'].'&amp;state=3">
										...
										echo'
										<span class="badge badge-success">'; if($mobile==0) {echo $nbres[0];} echo'</span>
									</a>
								</li>';
								echo '
								<li class="light-blue">
						...
						<div class="main-content">
							<div class="breadcrumbs" id="breadcrumbs">
								...
								<ul class="breadcrumb">
									<li>
										...
										if($_GET['page']=='plugins/availability/index') echo ' <span class="divider"><i class="icon-angle-right arrow-icon"></i></span><a href="index.php?page=plugins/availability/index">'.T_('Disponibilité').'</a>';
										if($_GET['page']=='admin' || $_GET['page']=='changelog') echo ' <span class="divider"><i class="icon-angle-right arrow-icon"></i></span> <a href="./index.php?page=admin">'.T_('Administration').'</a>';
										if($_GET['viewid']!='' || $_GET['page']=='view') echo ' <span class="divider"><i class="icon-angle-right arrow-icon"></i></span><a href="index.php?page=dashboard"> '.T_('Vues').'</a>';
										if($_GET['page']=='asset' || $_GET['page']=='asset_list') echo ' <span class="divider"><i class="icon-angle-right arrow-icon"></i></span><a href="index.php?page=asset_list"> '.T_('Équipements').'</a>';
										if($_GET['page']=='asset_stock') echo ' <span class="divider"><i class="icon-angle-right arrow-icon"></i></span><a href="index.php?page=asset_stock"> '.T_('Entrée en stock').'</a>';
										//second level
							...
							<div class="page-content">
								...
								)
								{
									...
									elseif ($_GET['page']=='calendar' && $rright['planning']==0) {echo '<div class="alert alert-danger"><strong><i class="icon-remove"></i>'.T_('Erreur').':</strong> '.T_('Vous n\'avez pas les droits d\'accès aux calendriers, contacter votre administrateur').'.<br></div>';}
